---
description: Writes and runs tests for a specific logitrack feature. Pass the spec name as argument e.g. /test-feature 05-backend-connection
...
Invoke the **logitrack-test-writer** subagent with the
following context:
...
Wait for logitrack-test-writer to fully complete and
...
Once logitrack-test-writer has finished, invoke the
**logitrack-test-runner** subagent with the following
context:
...
- If logitrack-test-writer reports it could not write
...
- Mirror the logitrack-test-runner's structured report
